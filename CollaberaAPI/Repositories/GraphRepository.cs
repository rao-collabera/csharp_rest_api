...
namespace CollaberaAPI
{
    ...
    public class GraphRepository : IGraphRepository
    {
        ...
        public GraphRepository(IConfiguration Config, IHttpContextAccessor Context)
        {
            _config = Config;
        ...
        public GraphServiceClient GetGraphClient()
        {
            ...
            var onBehalfOfCredential = new OnBehalfOfCredential(_config["AzureAd:tenantId"], _config["AzureAd:ClientId"], _config["AzureAd:ClientSecret"], accesstoken);
        ...
        public async Task<string> CalendarView(JsonElement payload)
        {
            ...
            var result = await graphClient.Me.CalendarView.GetAsync(x =>
            {
                ...
                x.QueryParameters.Select = new string[] { "subject", "bodyPreview", "organizer", "start", "end" };
                x.QueryParameters.StartDateTime = StartDate.ToString();
                x.QueryParameters.EndDateTime = EndDate.ToString();
                x.QueryParameters.Orderby = new string[] { "start/dateTime" };
            });
        ...
        {
            List<Recipient> recipientsList = new();
            foreach (var addr in recipients.Split(new System.Char[] { ',', ';' }))
            {
